...
# ========== GPU Decode Switch ==========
# Set to True to enable GPU decoding for testing
# Set to False to use CPU decoding (default)
USE_GPU_DECODE = False  # <-- Change this to True to test GPU decode
...
def get_frames_by_indices(
    ...
    if video_backend == "decord":
        ...
        # Apply GPU decode switch if no ctx specified
        if USE_GPU_DECODE and "ctx" not in video_backend_kwargs:
            ctx = gpu(GPU_DEVICE_ID)
        else:
            ctx = video_backend_kwargs.get("ctx", cpu(0))
