...
class DualBrainTrainer(transformers.Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        # Pass global_step to model for visualization synchronization
        inputs['global_step'] = self.state.global_step
        outputs = model(inputs)
        loss = outputs["loss"]
        
        # Log all metrics to TensorBoard at logging intervals
        if (self.state.global_step > 0 and 
            ...
            for k, v in outputs.items():
                if k != 'loss':  # loss is already logged by Trainer
    ...
    def create_optimizer(self):
        """
        Setup the optimizer with optional layer-wise learning rates.

        Supports backbone_lr for separate backbone learning rate.
        If backbone_lr is None, uses default learning_rate for all parameters (backward compatible).
        """
        ...
        if self.optimizer is None:
            ...
            # Get backbone_lr from model (None means use default learning_rate)
            backbone_lr = getattr(opt_model, 'backbone_lr', None)
            
            if backbone_lr is None:
                # Backward compatible: use original logic with single learning rate
                ...
                # Layer-wise learning rate: backbone uses backbone_lr, others use learning_rate
                ...
                for n, p in opt_model.named_parameters():
                    ...
                    if is_backbone:
                        (backbone_decay if is_decay else backbone_no_decay).append(p)
                    else:
                        (other_decay if is_decay else other_no_decay).append(p)
                
                optimizer_grouped_parameters = [
                    {"params": backbone_decay, "weight_decay": self.args.weight_decay, "lr": backbone_lr},
                    {"params": backbone_no_decay, "weight_decay": 0.0, "lr": backbone_lr},
                    {"params": other_decay, "weight_decay": self.args.weight_decay},
                    {"params": other_no_decay, "weight_decay": 0.0},
                ]
                # Filter out empty groups
                optimizer_grouped_parameters = [g for g in optimizer_grouped_parameters if g["params"]]
                
                print(f"[Layer-wise LR] backbone_lr: {backbone_lr}, other_lr: {self.args.learning_rate}")
