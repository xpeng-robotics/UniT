...
class LeRobotSingleDataset(Dataset):
    ...
    def __init__(
        ...
        relative_stats_filename: str = LE_ROBOT_RELATIVE_STATS_FILENAME,
        ...
        self._trajectory_ids, self._trajectory_lengths, self._trajectory_tasks = self._get_trajectories(split=split, episode_ids=episode_ids)
    ...
    def _get_metadata(self, embodiment_tag: EmbodimentTag) -> DatasetMetadata:
        ...
        if relative_stats_path.exists():
            ...
            for our_modality in ["state", "action"]:
                for subkey in simplified_modality_meta[our_modality]:
                    # Prefer direct LeRobot keys (e.g. IRON): action.relative.* / observation.state.relative.*
                    if our_modality == "state":
                        direct_key = f"observation.state.relative.{subkey}"
                    else:
                        direct_key = f"action.relative.{subkey}"
                    
                    if direct_key in relative_le_statistics:
                        relative_dataset_statistics[our_modality][f"relative.{subkey}"] = relative_le_statistics[direct_key]
                        continue

                    # Legacy key patterns (GR1, EgoDex, ...).
                    if not (subkey.endswith("pos") or subkey.endswith("rot") or subkey.endswith("rot6d")):
                        continue

                    prefix, suffix = subkey.rsplit("_", 1)
                    search_suffix = "rot" if suffix == "rot6d" else suffix
                    if our_modality == "state":
                        candidates = [f"observation.state.relative.{prefix}_in_world_{search_suffix}", f"observation.state.relative.{prefix}_in_wrist_{search_suffix}"]
                    else:
                        candidates = [f"action.relative.{prefix}_in_world_{search_suffix}", f"action.relative.{prefix}_in_wrist_{search_suffix}"]
    ...
    def _get_trajectories(self, split: str | None = None, episode_ids: list[int] | None = None) -> tuple[np.ndarray, np.ndarray, list | None]:
        """Get the trajectories in the dataset.
        
        Args:
            split: Python slice syntax string, e.g. "[:-10]", "[:100]"
                   Or "@/path/to/file.json" to load episode_ids from a JSON file.
            episode_ids: List of episode IDs to include. If provided, takes priority over split.
                         Can be loaded from a JSON file with train/val/test splits.
        
        Priority: episode_ids > split > None (use all)
        """
        ...
        # Support @file format: load episode_ids from JSON file
        if split is not None and split.startswith("@"):
            episode_ids_file = split[1:]  # Remove @ prefix
            with open(episode_ids_file, "r") as f:
                episode_ids = json.load(f)
            print(f"Loaded episode_ids from {episode_ids_file}: {len(episode_ids)} episodes")

        # Filter by episode_ids (highest priority)
        if episode_ids is not None:
            episode_ids_set = set(episode_ids)
            filtered_ids = []
            filtered_lengths = []
            filtered_tasks = [] if trajectory_tasks is not None else None
            
            for i, tid in enumerate(trajectory_ids):
                if tid in episode_ids_set:
                    filtered_ids.append(tid)
                    filtered_lengths.append(trajectory_lengths[i])
                    if trajectory_tasks is not None:
                        filtered_tasks.append(trajectory_tasks[i])
            
            trajectory_ids = filtered_ids
            trajectory_lengths = filtered_lengths
            trajectory_tasks = filtered_tasks
            print(f"Filtered by episode_ids, len({self.dataset_path}): {len(trajectory_ids)}")
        
        # Filter by split (Python slice syntax)
            ...
            print(f"Filtered by split '{split}', len({self.dataset_path}): {len(trajectory_ids)}")
...
class LeRobotMixtureDataset(Dataset):
    ...
    def compute_overall_statistics(
        ...
        for modality in modality_keys:
            # Weighted merge must use full tensor shapes for mean/variance (not scalar length).
            sample_mean = np.array(per_task_stats[0][modality]["mean"])
            data_shape = sample_mean.shape
    ...
    def update_metadata(self, metadata_config: dict) -> None:
        ...
        for tag, metadatas in all_metadatas.items():
            tag_indices = [i for i, d in enumerate(self.datasets) if d.tag == tag]
            tag_weights = [self.dataset_sampling_weights[i] for i in tag_indices]
