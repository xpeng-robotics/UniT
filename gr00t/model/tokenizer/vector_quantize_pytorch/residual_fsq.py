    ...
    def forward(
        ...
        with autocast('cuda', enabled = False):
            for quantizer_index, (layer, scale) in enumerate(zip(self.layers, self.scales)):
                ...
                # Always execute forward for DDP compatibility
                quantized, indices = layer(residual / scale)
                quantized = quantized * scale

                # For dropout layers: skip residual/quantized_out update but keep computation graph
                if is_dropout_layer:
                    # Add null indices for dropout layer
                    all_indices.append(null_indices)
                    # Don't update residual and quantized_out (maintain dropout semantics)
                    continue
