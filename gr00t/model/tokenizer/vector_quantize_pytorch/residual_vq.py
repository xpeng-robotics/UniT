...
class ResidualVQ(Module):
    ...
    def __init__(
        ...
        *,
        ...
        quantize_dropout_prob = 1.0,  # probability of applying dropout when enabled
        ...
        **vq_kwargs
        ...
        self.quantize_dropout_prob = quantize_dropout_prob  # probability of applying dropout
    ...
    def forward(
        ...
        if should_quantize_dropout:
            ...
            # apply dropout with probability quantize_dropout_prob
            if rand.random() >= self.quantize_dropout_prob:
        ...
        for quantizer_index, (vq, maybe_mlp) in enumerate(zip(self.layers, maybe_code_transforms)):
            ...
            # Check if this layer should be dropped (quantize_dropout)
            is_dropout_layer = should_quantize_dropout and quantizer_index > rand_quantize_dropout_index
            ...
            # (skip for dropout layers to maintain original behavior)

            if not is_dropout_layer:
                all_residuals = cat((all_residuals, rearrange(residual, '... d -> ... 1 d')), dim = -2)

            # vector quantize forward
            # Note: Always execute forward even for dropout layers (for DDP compatibility)
            # but mask the loss to 0 for dropout layers
            ...
            # For dropout layers: mask loss to 0 (keep computation graph) and skip residual update
            if is_dropout_layer:
                # Mask loss to 0 while keeping computation graph connected (for DDP)
                loss = loss * 0
                # Add masked loss and placeholder indices
                all_indices = cat((all_indices, torch.full_like(rearrange(embed_indices, '... -> ... 1'), -1)), dim = -1)
                all_losses = cat((all_losses, rearrange(loss, '... -> ... 1')), dim = -1)
                # Skip residual and quantized_out update (maintain dropout semantics)
                continue
